...
class Pages extends BaseController
{
	...
	public function blog()
	{
		...
		$data['data'] = $this->News->getNewssOrderTglUpdload();
		...
		$search = htmlspecialchars($this->request->getVar('cari'));
		$bulan = htmlspecialchars($this->request->getVar('bulan'));
		$tahun = htmlspecialchars($this->request->getVar('tahun'));
		$data['data'] = $this->News->search($search, $bulan, $tahun);
	...
	public function list()
	{
		...
		$search = htmlspecialchars($this->request->getVar('cari'));
		if ($search) {
			$data['judul'] = $this->News->search($search);
			// dd($data['judul']);
		}
	...
	public function academyList()
	{
		...
		$priceTag = array_map(function ($n) {
			return ($n > 0) ? 'premium' : 'free';
		}, $courseData['biaya']);

		// get instruktur data
		$instructurData = array_map(function ($n) {
			return $this->CourseInstructur->getCourseInstructur(['id' => $n]);
		}, $courseData['idStruktur']);
		...
		if (isset($_SESSION['userData'])) {
			...
			$data['course'] = (array_map(function ($data) use ($p) {
				unset($data[$p]);
